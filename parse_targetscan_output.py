...
######### USE CODE TO COBVERT SITE POSITIONS TO UTR AND CHROMO POSITIONS    
    
    
    # VERIFY CODE TO GET POSITIONS ON CHROMO FROM POSITIONS IN UTR
    # VERIFY CODE TO GO FROM COORD 0-INDEX BASED TO 1-INDEX BASED TO WRITE IN FILE
    
#                    MSA_start = int(line[3])
#                    MSA_end = int(line[4])
#                    start = line[5]
#                    end - line[6]
#                    
#                    # make a string with site information to use as key
#                    site = ':'.join([line[0], line[3], line[4], line[5], line[6], line[8]])
#                    
#                    header = ['transcript',
#                             'miRfamily_ID',
#                             'type_of_site',
...
#                             site_info = site.split(':')
#                             transcript = site_info[0]
#                             MSA_start = int(site_info[1])
#                             MSA_end = int(site_info[2])
#                             UTR_start = int(site_info[3])
#                             UTR_end = int(site_info[4])
#                             site_type = site_info[5]

       
#        # write chromosome location, UTR/downstream seq status and UTR coordinates {gene1 : [chromo, start, end, sense, UTR/downstream]}
#        utr_start = UTR_positions[transcript][1]
#         utr_end = utr_positions[gene][2]
#         utr_downstream = UTR_positions[transcript][-1]
         
#        # note that all positions to be recorded in the file are inclusive and start at index 1
#        # need to adjust the positions of the sites on the UTR when the UTR sens is -





#            # compute the positions of the site in the genome sequence (1-based index)
#            # if orientation == +
#            (start = site_UTR_start + start_UTR -1 ; end = site_UTR_end + start_UTR -1)
#            site_chromo_start = UTR_start + UTR_positions[transcript][1] - 1
#            site_chromo_end = UTR_end + UTR_positions[transcript][1] - 1
#            



#            # compute the positions of the site on the reverse complement of the UTR
#            # if orientation == -            
#            (start = len(UTR) -UTR_end + 1 ; end = len(UTR) - UTR_start + 1)
#            site_UTR_start = len(crm_UTR_seq[transcript].replace('-', '')) - UTR_end + 1 
#            site_UTR_end = len(crm_UTR_seq[transcript].replace('-', '')) - UTR_start + 1
#            site_chromo_start = site_UTR_start + UTR_positions[transcript][1] - 1
#            site_chromo_end = site_UTR_end + UTR_positions[transcript][1] - 1
#            # write site positions on the UTR
#            newfile.write(str(site_UTR_start) + '\t' + str(site_UTR_end) + '\t')
#            # write site positions in the genome sequence and UTR / site orienation
#            newfile.write(str(site_chromo_start) + '\t' + str(site_chromo_end) + '\t' + UTR_positions[transcript][3] + '\n')
#    # close file after writing
#    newfile.close()
...
# convert site positions to genome (all coord below are 0-based indices)
