...
newfile = open('CremClamiRNAOrthologs.txt')
for mir in CrmToCla:
    ...
    if '-5p' in crmmirna:
        assert '-5p' in clamirna, '5p arm in crm does not match arm in cla ortholog'
        crmmirna = crmmirna.replace('-5p', '_5p')
        clamirna = clamirna.replace('-5p', '_5p')
    elif '-3p' in crmmirna:
        assert '-3p' in clamirna, '3p arm in crm does not match arm in cla ortholog' 
...
#write header
header = ['name', 'chromo', 'hairpin_start', 'hairpin_end', 'mature_start', 'mature_end', 'mature', 'arm']
